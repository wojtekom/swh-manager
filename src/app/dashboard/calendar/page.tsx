...
interface CalendarSchedule {
  ...
  group: {
    id: string;
    name: string;
    category: string;        // AgeCategory (PZHL) — fallback
    swhCategory?: string;    // SwhCategory — preferowane
  };
}
...
interface CalendarSession {
  id: string;
  title: string;
  topic?: string | null;      // NOWE: temat dnia (np. „Krawędzie i hamowanie")
  date: string;
  duration: number;
  plan: {
    id: string;
...
interface DayEvent {
  ...
  title: string;
  subtitle?: string;       // np. temat dnia dla sesji
  time?: string;
  location?: string;
  href?: string;           // link do szczegółu (konspekt, turniej, obóz)
...
// Tailwind klasy per grupa (działają w light i dark mode)
...
/**
 * Mapuje stare AgeCategory (PZHL) na SwhCategory, jeśli nowe pole jest puste.
 * Po pełnej migracji backendu fallback ten przestanie być potrzebny.
 */
...
export default function CalendarPage() {
  ...
  function getEventsForDay(day: number): DayEvent[] {
    ...
    // Sesje treningowe (konkretne, z tematem dnia)
    ...
    if (typeFilter === "ALL" || typeFilter === "camp") {
      data.camps
        ...
        .forEach((c) => {
          ...
          // Obóz bez kategorii pokazujemy zawsze
  ...
  function toggleGroup(grp: SwhCategory) {
    setGroupFilters((prev) => {
      ...
      // jeśli wszystko wyłączone — przywróć wszystko
